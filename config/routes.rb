Webservices::Application.routes.draw do
  ...
  scope 'v1', module: 'api/v1', defaults: { format: :json } do
    concerns :v1_api_routable
  end
  scope module: 'api/v1', constraints: ApiConstraint.new(default: false, version: 1), defaults: { format: :json } do
    concerns :v1_api_routable
  end

  concern :v2_api_routable do
    path = { 'market_researches' => 'market_research_library',
             'parature_faq'      => 'faqs',
     }
    ...
    # Paths for the rest of the controllers are
    # the snake-case versions of their names.
    Dir[Rails.root.join('app/controllers/api/v2/*.rb').to_s].map do |filename|
      controller = File.basename(filename, '.rb').sub('_controller', '')
      path[controller] ||= controller
    end
  ...
  scope 'v2', module: 'api/v2', defaults: { format: :json } do
    concerns :v2_api_routable
  end
  scope module: 'api/v2', constraints: ApiConstraint.new(default: true, version: 2), defaults: { format: :json } do
    concerns :v2_api_routable
  end
end
