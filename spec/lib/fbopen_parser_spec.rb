...
describe FbopenParser do
  ...
  describe '#convert' do
    let(:srcsgt)          { open("#{fixtures_dir}/patch_source_input_srcsgt").read }
    ...
    let(:presol)          { open("#{fixtures_dir}/patch_source_input_presol").read }
    ...
    let(:duplicatekey)          { open("#{fixtures_dir}/patch_source_input_duplicatekey").read }
    ...
    let(:all_records)           { open("#{fixtures_dir}/patch_source_example_input").read }
    ...
    it 'works when given a file handle' do
      fh = open("#{fixtures_dir}/patch_source_example_input")
