...
describe 'Canada Leads API V2', type: :request do
  before(:all) do
  ...
  describe 'GET /canada_leads/search.json' do
    context 'when search parameters are empty' do
      before { get search_path, { size: 100 }, v2_headers }
      subject { response }
    ...
    context 'when q is specified' do
      before { get search_path, { q: 'engineer' }, v2_headers }
      subject { response }
    ...
    context 'when industry is specified' do
      before { get search_path, { industry: 'dental' }, v2_headers }
      subject { response }
    ...
    context 'when specific_location is specified' do
      before { get search_path, { specific_location: 'Manitoba' }, v2_headers }
      subject { response }
    ...
    context 'when searching for field with non ascii characters using ascii characters' do
      before { get search_path, { q: 'Montée' }, v2_headers }
      subject { response }
