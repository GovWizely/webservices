...
describe 'ITA Office Locations API V2', type: :request do
  ...
  describe 'GET /ita_office_locations/search.json' do
    context 'when q is specified' do
      ...
      before { get '/v2/ita_office_locations/search', params, @v2_headers }
      subject { response }
    ...
    context 'when country is specified' do
      before { get '/v2/ita_office_locations/search', { q: 'saN Jose', countries: 'Cr' }, @v2_headers }
      subject { response }
    ...
    context 'when USA and state is specified' do
      before { get '/v2/ita_office_locations/search', { q: 'san jose', countries: 'US', state: 'CA' }, @v2_headers }
      subject { response }
    ...
    context 'when just US state is specified' do
      before { get '/v2/ita_office_locations/search', { q: 'san jose', state: 'CA' }, @v2_headers }
      subject { response }
    ...
    context 'when country and city are specified' do
      before { get '/v2/ita_office_locations/search', { countries: 'cr', city: 'san jose' }, @v2_headers }
      subject { response }
